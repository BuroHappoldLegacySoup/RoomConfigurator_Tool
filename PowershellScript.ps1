...
$RoomConfigurator = Read-Host "`nPlease enter the software name."

try 
{
    # Replace occurrences of "RoomConfigurator" in all files
    Write-Host "`nReplacing occurrences of 'RoomConfigurator' with "$RoomConfigurator" in all files.`n"
    ...
    Get-ChildItem -File -Recurse | ForEach-Object {
        ...
        try
        {
            (Get-Content $_.FullName) -replace 'RoomConfigurator', $RoomConfigurator | Set-Content $_.FullName
        } 
        catch 
        {
    ...
    Write-Host "`nRenaming files and directories using "$RoomConfigurator":`n"
    ...
    # Get all files and directories containing "RoomConfigurator" recursively
    Get-ChildItem -Recurse -Directory | ForEach-Object {
        $dirpath = $_.FullName
        $dirname = Split-Path  $dirpath -Leaf
        if ($dirname.Contains("RoomConfigurator"))
        {
            $stack.Push($dirpath)
        ...
        foreach ($file in [Directory]::EnumerateFiles($dirpath)) 
        {
            ...
            if ($filename.Contains('RoomConfigurator') -and -not $allPaths.Contains($file))
            {
                $stack.Push($file)
                $allPaths.Add($file)
            }
    ...
    Get-ChildItem -File | ForEach-Object {
        if ($_.FullName.Contains("RoomConfigurator")) {
    ...
    while ($stack.Count) {
        ...
        if($filename.Contains('RoomConfigurator') -and $pathExists)
        {
            $newName = $filename.Replace('RoomConfigurator', $RoomConfigurator)
